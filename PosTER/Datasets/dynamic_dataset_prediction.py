...
class DynamicDatasetPrediction(Dataset):
    """
    ...
    def __init__(self, input_folder):
        ...
        self.path_images = glob(os.path.join(input_folder, '*.png'))+glob(os.path.join(input_folder, '*.jpg'))
        self.predictor_obj = PifPafPredictor()
        self.transforms_to_apply = transforms.Compose([NormalizeKeypointsRelative(), BodyParts()])
    ...
    def __getitem__(self, idx):
        ...
        for i, (pred_batch, _, meta_batch) in enumerate(tqdm(predicted_loader)):
            original_im = PIL.Image.open(open(meta_batch['file_name'], 'rb')).convert('RGB')
            im_size = original_im.size
            pifpaf_outs = {
            ...
            if self.transforms_to_apply:
                predicted_kps = self.transforms_to_apply(predicted_kps)
        return im_name, original_im, predicted_kps.float(), boxes